...
// Secret key from env only (never commit keys — Git push is blocked if sk_ is in the repo)
const stripe = process.env.STRIPE_SECRET_KEY
  ? require("stripe")(process.env.STRIPE_SECRET_KEY)
  : null;
router.use(bodyParser.json());
// Must match the hint shown on the register page
const PASSWORD_HINT =
  "At least 8 characters, and include an uppercase letter, a lowercase letter, and a number.";
  ...
  try {
    ...
    if (!validPass) {
      return res.status(400).json({
        error: `Password does not meet requirements. ${PASSWORD_HINT}`,
      });
    }
...
router.post("/create-payment-intent", async (req, res) => {
  if (!stripe) {
    return res.status(503).json({
      error: "Set STRIPE_SECRET_KEY in js_backend/.env (Stripe Dashboard → API keys).",
    });
  }
  try {
